...
Copyright::         	(c) 2008-2009, the University of Manchester and the European Bioinformatics Institute (EMBL-EBI)
License::           	BSD
Version::           	0.1.0
...
* A complete set of ActiveRecord models for annotations, annotations attributes and annotation value seeds.
...
* Create the following files in your application:
  ...
  * \AnnotationsController controller:
      ...
      require_dependency File.join(Rails.root, 'vendor, 'plugins', 'annotations', 'lib', 'app', 'controllers', 'annotations_controllers')
...
* When using the built in Annotations Controller to create annotations, only the current user (obtained from the _current_user_ method call in the controller) is set as the source of these Annotations. If you need a different source to be set, you can write your own Annotation creation methods in your app that directly creates the Annotation AR model.
